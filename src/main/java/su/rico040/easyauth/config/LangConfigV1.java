package su.rico040.easyauth.config;
...
public class LangConfigV1 extends ConfigTemplate{
    ...
    public static final class TranslatableText {
        ...
        public void send(CommandSourceStack commandOutput) {
            if (enabled && commandOutput != null) {
                if (langConfig.enableServerSideTranslation && serverSide) {
                    commandOutput.sendSystemMessage(translatable(key));
                } else {
                    commandOutput.sendSystemMessage(Component.literal(fallback));
                }
            }
        }
        ...
        public void send(CommandSourceStack commandOutput, Object... args) {
            if (enabled && commandOutput != null) {
                if (langConfig.enableServerSideTranslation && serverSide) {
                    commandOutput.sendSystemMessage(translatable(key, args));
                } else {
                    commandOutput.sendSystemMessage(translatable(fallback, args));
                }
            }
        }
        ...
        public MutableComponent getWithFallback() {
            if (enabled) {
                if (langConfig.enableServerSideTranslation && serverSide) {
        ...
        public MutableComponent getWithFallback(Object... args) {
            if (enabled) {
                if (langConfig.enableServerSideTranslation && serverSide) {
